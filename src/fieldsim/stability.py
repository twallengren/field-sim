...
advective flux          ``max_cell[(u_R^+ + u_L^- + v_U^+ + v_D^-)/dx]``
...
Each term's ``max_rate`` is the largest coefficient with which *any single
cell* is drained by that term.  For the donor-cell advective flux that is the
per-cell sum of the outflowing parts of **all four** of its faces,

    max over cells of  (u_R^+ + u_L^- + v_U^+ + v_D^-) / dx,

with ``x^+ = max(x, 0)``, ``x^- = max(-x, 0)`` and zero contribution from the
boundary faces.  The one-sided ``(max|u| + max|v|)/dx`` is *not* an upper bound
for this: where the attractant has an interior local minimum both faces of an
axis drain the same cell, and the one-sided form under-counts the true outflow
by up to a factor of 2 per axis.

Rates are summed **per target field** and the maximum is taken across fields.
In the standard one-diffusion-term-per-field case this reduces to
``4*alpha_max/dx**2 + max_cell[(u_R^+ + u_L^- + v_U^+ + v_D^-)/dx] + s_max``;
grouping by target also gets the food field's ``r + beta*max(P)`` right
automatically.
